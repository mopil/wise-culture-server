...
public class CommentService {
	...
	public Comment editComment(Long id, String content, Member writer) {
		if(commentRepository.existsById(id) == false) throw new CommentException("존재하지 않는 댓글");
		if(commentRepository.getById(id).getMember().getId() != writer.getId()) throw new CommentException("다른 회원의 댓글");
