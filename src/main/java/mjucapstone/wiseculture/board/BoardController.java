...
public class BoardController {
	...
	// 게시글 작성
	@PostMapping("")
	public ResponseEntity<?> write(@Valid @RequestBody BoardForm boardForm, BindingResult bindingResult, @Login Member loginMember) {
	...
	// 게시글 수정
	@PutMapping("/{id}")
	public ResponseEntity<?> edit(@PathVariable Long id, @Valid @RequestBody BoardForm boardForm, BindingResult bindingResult, @Login Member loginMember) {
		if(bindingResult.hasErrors()) return badRequest(convertJson(bindingResult.getFieldErrors()));
	...
	// 게시글 삭제
	@DeleteMapping("/{id}")
	public ResponseEntity<?> delete(@PathVariable Long id, @Login Member loginMember) {
